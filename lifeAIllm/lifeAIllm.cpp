...
void send_message(void* socket, const char* message) {
    zmq_msg_t hmsg;
    zmq_msg_t msg;
    zmq_msg_init_size(&hmsg, strlen(header));
    zmq_msg_init_size(&msg, strlen(message));
    memcpy(zmq_msg_data(&hmsg), header, strlen(header));
    memcpy(zmq_msg_data(&msg), message, strlen(message));
    zmq_msg_send(&hmsg, socket, ZMQ_SNDMORE);
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (input_echo) {
            for (auto id : embd) {
                ...
                send_message(socket, token_str.c_str());
